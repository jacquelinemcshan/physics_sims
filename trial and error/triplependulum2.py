...
def fetch_data_from_user_input(input_value, input_value2, input_value3, input_value4,
    ...
    if ctx.triggered_id == "graph-button":
        ...
        fig = go.Figure(frames=[go.Frame(data=[go.Scatter(x=[x1[k]],y=[y1[k]],mode="markers",name='mass 1',
                                ...
                                go.Scatter(x=[0, x1[k], None, x1[k], x2[k],None, x2[k],x3[k]],y=[0, y1[k], None, y1[k], y2[k],None, y2[k],y3[k]], name='string',mode="lines", 
        ...
        fig.add_trace(go.Scatter(x=x2, y=y2, name='mass 2',mode="lines", line=dict(color="red", width=2)))
        fig.add_trace(go.Scatter(x=x3, y=y3,  name='mass 3',mode="lines",line=dict(color="green", width=2)))
        fig.add_trace(go.Scatter(x=[0, x1, None, x1, x2, None, x2,x3],y=[0, y1, None, y1, y2,None, y2,y3], name='string', mode="lines", 
                                 line=dict(color="black", width=1), showlegend=False))
